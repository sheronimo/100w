...
function cust_display_sermon_series($pod_id) {
	...
	if (!empty($child_terms)) {
	  $html = '<ul class="subcats-dropdown">';

	  foreach ($child_terms as $child_term) {
		$html .= '<li class="js-cat cat-child"><span>' . $child_term->name . '</span></li>';
	  }
...
function cust_display_verse_field($pod_id) {
	$chapter_names = wp_get_post_terms($pod_id, 'bible_chapter');
	
	if (count($chapter_names) == 1) {
		$chapter1 = $chapter_names[0]->name;
		$verse1 = get_post_meta($pod_id, 'first_verse', true);
		
		$output = '<span>' . $chapter1 . ':' . $verse1 . '</span>';
	} else if (count($chapter_names) == 2) {
		$chapter1 = $chapter_names[0]->name;
		$verse1 = get_post_meta($pod_id, 'first_verse', true);

		$chapter2 = $chapter_names[1]->name;
		$verse2 = get_post_meta($pod_id, 'second_verse', true);

		$output = '<span>' . $chapter1 . ':' . $verse1 . ' to ' . $chapter2 . ':' . $verse2 . '</span>';
	}
